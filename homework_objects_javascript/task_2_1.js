...
function unique(persons){
  const uniquePersons = []
  const uniquePersonsObjects = [persons[0]]
  uniquePersons.push(JSON.stringify(persons[0]))
  for (let i =1; i< persons.length; i++){
      if(!uniquePersons.includes(JSON.stringify(persons[i]))){
          uniquePersonsObjects.push(persons[i])
          uniquePersons.push(JSON.stringify(persons[i]))
      }
  }
  return uniquePersonsObjects
}
